...
int lastLeftTimestamp = 0;
int lastRightTimestamp = 0;
...
void followLine()
{
  ...
  else if(digitalRead(right_edge) == LOW)
  {
    mspeed = 90;
    course = 3;    
    lastRightTimestamp = millis();
  }
  else if(digitalRead(left_edge) == LOW)
  {
    if (lastRightTimestamp + 500 > millis()) {
      mspeed = 90;
      course = 4;
      lastLeftTimestamp = millis();
    } else {
      course = 1;
    }l
  }
...
void followLine2()
{
  mdelay = 200;
  mspeed = 90;
  if(digitalRead(right_edge) == LOW && digitalRead(left_edge) == LOW)
  {
    course = 1;
  }
  ...
  else if(digitalRead(left_edge) == HIGH)
  {
    course = 4;
  }
  driveMotors();  
